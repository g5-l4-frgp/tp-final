	...
	public void AltaPrestamo(Prestamos prestamo, String fecha) {
		...
		Connection cn = null;
		  try
		  {
			 ...
			 cst.setInt(2, prestamo.getMovimiento().getIdMovimiento());
			 cst.setFloat(3, prestamo.getImporteConIntereses());
			 cst.setFloat(4, prestamo.getImportePedido());
			 cst.setFloat(5, prestamo.getPagoMensual());
			 cst.setDate(6, java.sql.Date.valueOf(fecha));
			 cst.setInt(7, prestamo.getCantidadCuotas());
			 cst.setInt(8, prestamo.getCuenta().getID());
			 cst.setInt(9, prestamo.getEstado());
			 cst.execute();   
		  }
		  catch (Exception e) {
			e.printStackTrace();
			...
			ArrayList<Prestamos> x = new ArrayList<Prestamos>();		
				Connection cn = null;
					try {
						...
						cn = DriverManager.getConnection(host+dbName, user,pass);
						CallableStatement st = cn.prepareCall("CALL SP_BuscarPrestamoxIdUsuario(?)");

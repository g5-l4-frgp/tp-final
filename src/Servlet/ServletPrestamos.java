...
//				}
				...
				if(request.getParameter("btnPagarCuotas")!=null) {
					...
					CuentaNegocio negocioCuenta=new CuentaNegocio();
					cuenta=negocioCuenta.BuscarCuentasXID(idCuenta);
					cuotas=negociocuota.lista_CuotasxID(idCuota);
					if((cuenta.getSaldo()<cuotas.getImporte()))
					{
						
						 alerta=("No podes pagar pobre");
					...
					else 
					{
						...
						cuenta.setSaldo(cuenta.getSaldo()-cuotas.getImporte());
						negocioCuenta.ModificarCuenta(cuenta);
						cuotas.setEstado(3);
						System.out.println(cuotas.getEstado());
						System.out.println(cuotas.getCuota());
						negociocuota.ModificarCuota(cuotas);
						alerta=("Se hizo el pago,ricardo ford");
					}
